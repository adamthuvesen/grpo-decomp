...
is **mostly elicitation** of latent base capability. The controls point away from new
reasoning, contamination, and formatting. Two corrections to the single-seed read: the seed-0
...
- **The placebo does not help.** Random is flat at ~74.5-77.0% across every seed
...
- **correct pass@8 ≈ base pass@8**: the between-seed interval is [+0.4, +1.1], but it holds the
  ...
  term, so the propagated interval **[−0.4, +1.9] is consistent with zero**. The verdict
...
- **All three clear zero after Holm** (3/3 significant, FWER-controlled): the gain survives the
  contamination, adversarial-perturbation, and label-noise checks.
- **The seed-0 numbers were highs.** gsm-symbolic settled **+10.5 → +4.1** (seed 0 was +10.5,
  seeds 1-5 are +1.8 to +4.4); gsm-plus +4.9 → +3.5; platinum +4.8 → +3.2. This is a third confident
...
Two follow-ups remain: find genuine expansion on *math*, and harden the placebo.
